...
class Hyperparameters:
    ...
    hadamard_export_enabled = bool(int(os.environ.get("HADAMARD_EXPORT_ENABLED", "1")))
    hadamard_export_chunk = int(os.environ.get("HADAMARD_EXPORT_CHUNK", 512))
...
def _named_rademacher(name: str, size: int, device: torch.device) -> Tensor:
    idx = torch.arange(size, dtype=torch.int64, device=device)
    seed = _stable_name_seed(name)
    bits = ((idx * 1103515245 + seed) >> 7) & 1
...
def hadamard_rotate_blocks(t: Tensor, name: str, chunk: int) -> Tensor:
    if t.ndim != 2 or chunk <= 0 or t.shape[-1] % chunk != 0:
        return t
    x = t.float().reshape(t.shape[0], -1, chunk)
    signs = _named_rademacher(name, chunk, x.device).view(1, 1, chunk)
    x = x * signs
    x = _fwht_last_dim(x) * (1.0 / math.sqrt(chunk))
    return x.reshape_as(t)


def hadamard_inverse_blocks(t: Tensor, name: str, chunk: int) -> Tensor:
    if t.ndim != 2 or chunk <= 0 or t.shape[-1] % chunk != 0:
        return t
    x = t.float().reshape(t.shape[0], -1, chunk)
    signs = _named_rademacher(name, chunk, x.device).view(1, 1, chunk)
...
def should_hadamard_precondition(name: str, t: Tensor, cat: str, chunk: int, enabled: bool) -> bool:
    return (
        enabled
        and cat in {"mlp", "attn"}
        and t.ndim == 2
        and chunk > 0
        and t.shape[-1] % chunk == 0
        and t.numel() > 65536
    )
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    hadamard_enabled: bool = True,
    hadamard_chunk: int = 512,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            preconditioned = should_hadamard_precondition(name, t, cat, hadamard_chunk, hadamard_enabled)
            q_src = hadamard_rotate_blocks(t, name, hadamard_chunk) if preconditioned else t
            q, s = quantize_int6_per_row(q_src)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {
                "type": "int6",
                "pre": "hadamard" if preconditioned else "identity",
                "chunk": hadamard_chunk if preconditioned else 0,
            }
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("pre") == "hadamard":
            t = hadamard_inverse_blocks(t, name, int(info.get("chunk", 0)))
...
def main() -> None:
    ...
    log0(
        f"hadamard_export:enabled:{args.hadamard_export_enabled} "
        f"chunk:{args.hadamard_export_chunk}"
    )
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        hadamard_enabled=args.hadamard_export_enabled,
        hadamard_chunk=args.hadamard_export_chunk,
    )
    hadamard_count = sum(
        1
    ...
    log0(f"hadamard_export:tensors:{hadamard_count}")
