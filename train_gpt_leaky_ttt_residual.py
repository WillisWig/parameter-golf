...
class Hyperparameters:
    ...
    residual_bigram_prior = bool(int(os.environ.get("RESIDUAL_BIGRAM_PRIOR", "1")))
    residual_bigram_prior_buckets = int(os.environ.get("RESIDUAL_BIGRAM_PRIOR_BUCKETS", 128))
    residual_bigram_prior_dim = int(os.environ.get("RESIDUAL_BIGRAM_PRIOR_DIM", 16))
    residual_bigram_prior_gate_init = float(os.environ.get("RESIDUAL_BIGRAM_PRIOR_GATE_INIT", 0.05))
...
class ResidualBigramPrior(nn.Module):
    def __init__(self, buckets: int, dim: int, vocab_size: int, gate_init: float):
        super().__init__()
    ...
    def bigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out = torch.empty_like(t)
        out[..., 0] = mod
        out[..., 1:] = torch.bitwise_xor(14653 * t[..., 1:], 31469 * t[..., :-1]) % mod
        return out.long()

    def forward(self, token_ids: Tensor) -> Tensor:
        logits = self.proj(self.embed(self.bigram_hash(token_ids)))
        return logits * torch.tanh(self.gate.to(dtype=logits.dtype))
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        residual_bigram_prior: bool = False,
        residual_bigram_prior_buckets: int = 128,
        residual_bigram_prior_dim: int = 16,
        residual_bigram_prior_gate_init: float = 0.05,
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        ln_scale: bool = False,
        dtg: bool = False,
        ...
        self.residual_bigram_prior = (
            ResidualBigramPrior(
                residual_bigram_prior_buckets,
                residual_bigram_prior_dim,
                vocab_size,
                residual_bigram_prior_gate_init,
            )
            if residual_bigram_prior
            else None
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.residual_bigram_prior is not None:
            logits_proj = logits_proj + self.residual_bigram_prior(input_ids).reshape(-1, logits_proj.size(-1))
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if self.residual_bigram_prior is not None:
            logits_proj = logits_proj + self.residual_bigram_prior(input_ids)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        residual_bigram_prior=args.residual_bigram_prior,
        residual_bigram_prior_buckets=args.residual_bigram_prior_buckets,
        residual_bigram_prior_dim=args.residual_bigram_prior_dim,
        residual_bigram_prior_gate_init=args.residual_bigram_prior_gate_init,
    ...
    if base_model.residual_bigram_prior is not None:
        scalar_params.append(base_model.residual_bigram_prior.gate)
    ...
    if base_model.residual_bigram_prior is not None:
        tok_params.append(
            {
                "params": [base_model.residual_bigram_prior.embed.weight],
                "lr": token_lr,
                "base_lr": token_lr,
            }
        )
        scalar_params.append(base_model.residual_bigram_prior.proj.weight)
    ...
    log0(
        f"residual_bigram_prior:enabled:{args.residual_bigram_prior} "
        f"buckets:{args.residual_bigram_prior_buckets} "
        f"dim:{args.residual_bigram_prior_dim} "
        f"gate_init:{args.residual_bigram_prior_gate_init}"
    )
