...
class Hyperparameters:
    ...
    synapse_cache_enabled = bool(int(os.environ.get("SYNAPSE_CACHE_ENABLED", "0")))
    synapse_cache_buckets = int(os.environ.get("SYNAPSE_CACHE_BUCKETS", 4096))
    synapse_cache_scale = float(os.environ.get("SYNAPSE_CACHE_SCALE", 0.20))
    synapse_cache_batch_seqs = int(os.environ.get("SYNAPSE_CACHE_BATCH_SEQS", 64))
...
def eval_val_synapse_cache(
    ...
    seq_len = eval_seq_len or args.train_seq_len
    batch_seqs = max(1, args.synapse_cache_batch_seqs)
    total_seqs = (val_tokens.numel() - 1) // seq_len
    seq_start = (total_seqs * rank) // world_size
    seq_end = (total_seqs * (rank + 1)) // world_size
    val_loss_sum = torch.zeros((), device=device, dtype=torch.float64)
...
def main() -> None:
    ...
    log0(
        f"synapse_cache:enabled:{args.synapse_cache_enabled} "
        f"buckets:{args.synapse_cache_buckets} "
        f"scale:{args.synapse_cache_scale} "
        f"batch_seqs:{args.synapse_cache_batch_seqs}"
    )
    ...
    if args.synapse_cache_enabled:
        ...
        cache_val_loss, cache_val_bpb = eval_val_synapse_cache(
        ...
        torch.cuda.synchronize()
        log0(
            f"final_synapse_cache_roundtrip val_loss:{cache_val_loss:.4f} val_bpb:{cache_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_cache):.0f}ms"
        )
        log0(f"final_synapse_cache_roundtrip_exact val_loss:{cache_val_loss:.8f} val_bpb:{cache_val_bpb:.8f}")
        log0(f"final_int8_zlib_roundtrip_exact val_loss:{cache_val_loss:.8f} val_bpb:{cache_val_bpb:.8f}")
